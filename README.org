...
7. Call ~activity-new~ with a universal prefix argument (~C-u C-x C-a n~) to redefine an activity's default state.
8. Suspend the activity with ~activity-suspend~ (~C-x C-a s~) (which saves its last state and closes its frame/tab).
